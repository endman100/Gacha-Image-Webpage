...
function openDetailView(loraName) {
    ...
    // 第二行：通用角色場景（獨立布局，放在 Workflow 設置之上）
    ...
    html += '<thead><tr><th>場景設計</th><th>維持衣服設計</th><th>prompt</th><th>操作</th></tr></thead>';
    ...
    // 第三行：專用角色場景（獨立布局，放在 Workflow 設置之上）
    html += '<div class="detail-row">';
    html += '<h3>🧩 專用角色場景</h3>';
...
function appendSceneRow(tbody, row) {
    ...
    tr.append(scenesTd, keepTd, promptTd, actionTd);
...
function collectPromptsFromSceneTableBody(tbody) {
    if (!tbody) {
        return [];
    }
...
async function runWorkflowWithRunpod(loraName) {
    ...
    const generalPrompts = collectPromptsFromSceneTableBody(document.getElementById('generalScenesTbody'));
    const characterPrompts = collectPromptsFromSceneTableBody(document.getElementById('characterScenesTbody'));
    const prompts = [...generalPrompts, ...characterPrompts];

    if (prompts.length === 0) {
        showRunpodStatus('請先在「通用角色場景 / 專用角色場景」填入至少 1 個 prompt', 'error');
        return;
    }

    const totalRounds = prompts.length * times;
    const jobs = [];
    ...
    if (progressUI) {
        ...
        progressUI.setSummary(`準備執行 ${totalRounds} 次 Workflow...`, {
            total: totalRounds,
            finished: 0,
            success: 0,
            failed: 0,
        ...
        showRunpodStatus(`準備執行 ${totalRounds} 次 Workflow...`, 'loading');
    }
    ...
    try {
        ...
        function renderProgress(message, type = 'loading') {
            ...
            showRunpodStatus(
                `進度 ${finishedCount}/${totalRounds}（成功 ${uploadedCount}，失敗 ${failedCount}，進行中 ${runningCount}，等待 ${waitingCount}）`,
                type
            );
        }
        ...
        async function runSingleRound(round, job) {
            ...
            renderProgress(`第 ${round}/${totalRounds} 次：送出 Runpod 任務...${detail ? `（${detail}）` : ''}`);
            if (progressUI) {
            ...
            const promptAdjustedWorkflowInput = applyCheckpointAndPromptOverrides(normalizedWorkflowInput, features, prompt);
            ...
            renderProgress(`第 ${round}/${totalRounds} 次：Runpod 任務執行中`);
            if (progressUI) {
            ...
            console.log(`第 ${round}/${totalRounds} 次 Runpod 輸出結果:`, output, '提取的圖片資訊:', imageInfo, '最終下載 URL:', imageSrc);

            if (!imageSrc) {
                throw new Error(`第 ${round}/${totalRounds} 次：任務完成，但找不到最後生成圖片`);
            }
            ...
            uploadedCount += 1;
            renderProgress(`第 ${round}/${totalRounds} 次完成，已上傳 ${uploadedCount}/${totalRounds} 張到 /${uploadPath}`);
            if (progressUI) {
        ...
        async function worker() {
            while (true) {
                ...
                nextRound += 1;
                runningCount += 1;
                renderProgress(`第 ${round}/${totalRounds} 次：已加入執行`);
                if (progressUI) {
                ...
                try {
                    ...
                    renderProgress(`第 ${round}/${totalRounds} 次失敗`);
                    if (progressUI) {
        ...
        if (failedCount > 0) {
            ...
            renderProgress(`✗ Workflow 完成（成功 ${uploadedCount}/${totalRounds}，失敗 ${failedCount}）。${preview}`, 'error');
        } else {
            renderProgress(`✓ Workflow 全部完成，已上傳 ${uploadedCount} 張圖片（共 ${totalRounds} 次）`, 'success');
        }
    } catch (error) {
...
function applyCheckpointAndPromptOverrides(workflowInput, features, prompt) {
